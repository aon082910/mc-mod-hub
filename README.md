...
- Searches **Modrinth**, **CurseForge**, **PlanetMinecraft**, **9Minecraft**, **BetterBedrock**, **Hangar**, **SpigotMC**, **TLMods**, **MinecraftSkins.net**, **Polymart**, **GitHub**, **MCreator.net**, and **McpeDude**, merging results into one table (styled after LimeTorrents' layout: category pills + search bar, green nav, notice board, sidebar quick-browse)
...
- **Notice board** on the homepage shows newly-posted mods/addons, pulled live from Modrinth's "newest" feed, 9Minecraft's and McpeDude's blog homepages, and CurseForge's newest listings (if a key is configured) — "Show more" expands it from 8 to 25 per source
- Every result and every mod detail page shows the **required game edition (Java or Bedrock)** and the **game version(s) it supports**, plus a **Posted** and **Updated** date column where the source publishes one (a source that doesn't — mainly the scraped, non-API ones — shows a plain dash rather than a guess)
- Results table is **sortable by every column** (click a header; click again to flip direction) and **filterable** (name, source, edition, version, minimum downloads, posted-after/updated-after) — all client-side over the current result set, no re-search needed
...
- **McpeDude** — a Bedrock-only mods/addons/texture-packs/maps blog. Unlike BetterBedrock/MinecraftSkins.net/MCreator.net above, its `?s=` search is real and confirmed working (a nonsense query returns zero result cards while a real one returns several, with a materially different response — not the same-output no-op pattern those three have). `robots.txt` only disallows `/wp-admin/`, so the search path is fair game. It's also the only scraped/blog-style source that publishes a real, structured publish date on its listing cards, so it's included in the "Posted" column and the notice board, not just null there like the others
...
- Search page: type a mod name → results merge Modrinth, CurseForge, PlanetMinecraft, 9Minecraft, BetterBedrock, Hangar, SpigotMC, TLMods, Polymart, GitHub, MCreator.net, and McpeDude, sorted by download count where available (sources without a public download counter sort after ones that have one). MinecraftSkins.net only appears when browsing the dedicated Skins category (see above — it has no real keyword search)
...
- Posted/Updated dates come from each source's own API field (Modrinth's `date_created`/`date_modified`, CurseForge's `dateCreated`/`dateModified`, GitHub's `created_at`/`pushed_at`, Hangar's `createdAt`/`lastUpdated`, Spiget's `releaseDate`/`updateDate`, Polymart's `creationTime`/`lastUpdateTime`) or, for McpeDude, its listing page's own published-date field — the other scraped sites (PlanetMinecraft, 9Minecraft, BetterBedrock, TLMods, MinecraftSkins.net, MCreator.net) don't publish either on their listing pages, so both columns show a dash there rather than a fabricated date. GitHub's "posted" date is when the *repository* was created, which can predate or postdate the mod's real first release depending on the project's history — it's the closest honest proxy available, not a guarantee
