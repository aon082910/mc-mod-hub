...
- Posted/Updated dates come from each source's own API field (Modrinth's `date_created`/`date_modified`, CurseForge's `dateCreated`/`dateModified`, GitHub's `created_at`/`pushed_at`, Hangar's `createdAt`/`lastUpdated`, Spiget's `releaseDate`/`updateDate`, Polymart's `creationTime`/`lastUpdateTime`) — the scraped, non-API sites (PlanetMinecraft, 9Minecraft, BetterBedrock, TLMods, MinecraftSkins.net, MCreator.net) don't publish either on their listing pages, so both columns show a dash there rather than a fabricated date. GitHub's "posted" date is when the *repository* was created, which can predate or postdate the mod's real first release depending on the project's history — it's the closest honest proxy available, not a guarantee
